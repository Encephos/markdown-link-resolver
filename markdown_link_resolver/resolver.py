...
def _url_to_base64(url: str) -> str:
    ...
    try:
        # Ein User-Agent ist wichtig, da einige Server blocken, wenn man keinen mitschickt
        req = urllib.request.Request(url, headers={'User-Agent': 'MarkdownLinkResolver/1.1'})
        with urllib.request.urlopen(req, timeout=5) as response:
            data = response.read()
            # MIME-Type (z.B. image/png) anhand der Endung raten
        ...
        # Wenn der Download fehlschlägt (z.B. 404 oder Timeout), 
        # geben wir die URL einfach unverändert zurück.
        return url

def resolve_markdown_links(markdown_text: str, base_url: str, inline_images: bool = False) -> str:
    """
    ...
    def md_repl(match):
        ...
        if inline_images and text_part.startswith('!') and new_url.startswith('http'):
            new_url = _url_to_base64(new_url)
